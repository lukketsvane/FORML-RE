...
\begin{tikzpicture}[x=0.78cm,y=0.62cm]
  ...
  \node[node-o] (p512)   at (10, -0.3)   {5.12};
  \node[node-i] (p5121)  at (12.2, -0.3) {5.121};
  \node[node-i] (p5122)  at (14.2, -0.3) {5.122};
  \node[node-i] (p5123)  at (16.2, -0.3) {5.123};
  \node[node-i] (p5124)  at (18.2, -0.3) {5.124};
  \node[node-i] (p5125)  at (20.2, -0.3) {5.125};
  \node[node-t] (p5126)  at (22.6, -0.3) {5.126};
  \draw[impl] (p5)   -- (p51);
  \draw[impl] (p51)  -- (p511);
  \draw[impl] (p511) -- (p5111);
  \draw[impl] (p511) to[bend right=10] (p512);
  ...
  % BAND 5: syntese (kjerne-likning og reduksjonsteorem)
  \node[node-d] (p57)    at (12, -10.5)  {5.7};
  \node[node-o] (p571)   at (15, -10.5)  {5.71};
  \node[node-t] (p572)   at (18, -10.5)  {5.72};
  \node[node-t] (p5721)  at (21, -10.5)  {5.721};
  \node[node-t] (p5722)  at (24, -10.5)  {5.722};
  \node[node-a] (p5723)  at (26.8, -10.5){5.723};
...
Seksjon 5 opnar med postulatet \emph{5.1} (triangel): kvar klasse har minst eitt system som navigerer landskapet via negativ tilbakekopling. Frå dette bygger \emph{5.11} den substrat-uavhengige definisjonen av \textbf{agent} som trippelet måltilstand, avstandsmåling, justeringsmekanisme.

Band 1 er stratifisert langs fem illustrasjonar (romber \emph{5.121} til \emph{5.125}): termostat, tre, koloni, pattedyr, designar. Kvar illustrasjon er eit døme på same struktur med ulik oppløysing. Designaren sin særprega posisjon står i \emph{5.126}: han navigerer på vegne av andre.

Band 2 introducerer \textbf{kognitiv lyskjegle} (\emph{5.2}) med formell KL-divergensdefinisjon (\emph{5.212}).
\end{minipage}\hfill
\begin{minipage}[t]{0.325\textwidth}
...
Band 4 er seksjonens rikaste band statistisk. Omhug (\emph{5.6}), intelligens (\emph{5.62}) og K-metrikken (\emph{5.621}) står som tre definisjonar i éi rekke. Liv-definisjonen (\emph{5.65}) hengjer ned som eigen underseksjon med tre teorem om levande, ikkje-levande og grensa mellom dei.

Band 5 er syntesepunktet. \emph{5.7} er kjerne-likninga, og han har \emph{tre foreldre}: agentar (\emph{5.1}), lyskjegle (\emph{5.2}) og grammatikk (\emph{5.3}). Dette er traktaten sin største konfluens.
\end{minipage}\hfill
\begin{minipage}[t]{0.325\textwidth}
{\scshape\small Strukturelle innsikter}

\emph{Tripleteforeldreskap.} \emph{5.7} er den einaste noden i traktaten med tre direkte foreldre. Dette er oppfinningssjangerens høgdepunkt; tre uavhengige liner (agent, lyskjegle, grammatikk) samlast i éi likning.

\emph{Reduksjonsteoremet som kjede.} \emph{5.72} skulle per notasjonen kunne markerast med dobbelpil sidan han formulerer ein ekvivalens under grensevilkår. Me har valt å representere ekvivalensen som teoremkjede (\emph{5.72} til \emph{5.723}) framfor dobbelpil, fordi dobbelpila ville krevd ein lang kryssande stig gjennom heile band 4.

\emph{To postulat.} \emph{5.1} (agens eksisterer) og \emph{5.723} (likninga lukkar feltet). Det siste er meta-teoretisk; det seier at ingen fleire lag kan leggjast til.
...
\emph{Sjanger stadfesta.} Oppfinningseksjonane (1, 2, 5) har alle konfluens. Karakteriseringseksjonane (3, 4) har ingen. Seksjon 5 tek oppfinningsforma til sitt ytterpunkt.
\end{minipage}
