...
Band 2 karakteriserer landskapets struktur langs to aksar. Til venstre: topologien av \textbf{lokale maksima} (\emph{3.12}) og den empiriske knusinga av den universelle toppen (\emph{3.121}). Til høgre: \textbf{stabiliteten} som funksjon av dalveggens brattleik (\emph{3.2, 3.21}). Kanalisering langs ein akse er formelt identisk med kort minnehalveringstid $\tau_{\mathrm{mem}}(X)$; djupe kanalar har kort $\tau_{\mathrm{mem}}$, grunne kanalar har lang. Ornstein--Uhlenbeck-formalismen operasjonaliserer 3.2 og 4.3 samstundes.
